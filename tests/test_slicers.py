...
def test_probe_accepts_a_binary_that_prints_flags(tmp_path):
    b = _fake_binary(tmp_path, "good",
                     'echo "OrcaSlicer 2.4.2"; echo " --slice"; '
                     'echo " --load-settings"; echo " --load-filaments"; '
                     'echo " --export-3mf"')
    r = slicers.probe(b)
    assert r["ok"] is True
...
def test_probe_rejects_a_binary_that_cannot_load_profiles(tmp_path):
    b = _fake_binary(tmp_path, "partial",
                     'echo "Some Slicer 1.0"; echo " --slice"; echo " --export-3mf"')
    r = slicers.probe(b)
    assert r["ok"] is False
    assert "--load-settings" in r["reason"]
